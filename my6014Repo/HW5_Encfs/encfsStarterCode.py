...
class EncFS(Operations):
    ...
    def getattr(self, path, fh=None):
        ...
        if path in self.openFiles:
            content = self.openFiles[path]
            props['st_size'] = len(content)
...
'''
    ...
    def create(self, path, mode, fi=None):
        ...
        fd = os.open(self._full_path(path), os.O_WRONLY | os.O_CREAT, mode)
        self.openFiles[path] = b''
        os.close(fd)
    ...
    def read(self, path, length, offset, fh):
        ...
        """
        content = self.openFiles[path]
        return_content = content[offset:length]
        if return_content == b'':
            return 0
        else:
    ...
    def write(self, path, buf, offset, fh):
        ...
        content = self.openFiles[path][:offset] + buf
        self.openFiles[path] = content
        return len(buf)
   ...
   '''                                            
    ...
    def release(self, path, fh):
        ...
        salt = os.urandom(16)
        token = bytes(self.openFiles[path])
