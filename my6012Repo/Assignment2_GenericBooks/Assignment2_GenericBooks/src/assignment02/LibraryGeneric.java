...
public class LibraryGeneric<Type> {
    ...
    public Type lookup(long isbn) {
        for( LibraryBookGeneric<Type> el: library ){
            if( el.getIsbn() == isbn ){
                return el.getHolder();
            }
        }
    ...
    public boolean checkout(long isbn, Type holder, int month, int day, int year) {
        for( LibraryBookGeneric<Type> el: library ){
            if( el.getIsbn() == isbn && el.isCheckedIn ){
                GregorianCalendar dueDate = new GregorianCalendar(year, month, day);
                el.checkOut(holder, dueDate);
                return true;
            }
        }
        return false;
    ...
    public boolean checkin(Type holder) {
        boolean hasHolder = false; // to record if checked in any books with specified holder
        for( LibraryBookGeneric<Type> el: library ){
            if( el.getHolder() != null && el.getHolder().equals(holder) ){
                el.checkIn();
                hasHolder = true;
    ...
    public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
        ...
        // save due date in the same type as due date in books
        GregorianCalendar dueDate = new GregorianCalendar( year, month, day );

        // add books that due date is older than input date into overdueList
        for( LibraryBookGeneric<Type> el: library ){
            if( el.getDueDate() != null && el.getDueDate().compareTo(dueDate) < 0 ){
                overdueList.add(el);
            }
        }

        // sort the overdueList
    ...
    protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
        /**
         * Returns a negative value if lhs name string is smaller than rhs. Returns a positive
         * value if lhs is larger than rhs. Returns 0 if lhs and rhs are equal.
         */
        public int compare( LibraryBookGeneric<Type>lhs, LibraryBookGeneric<Type>rhs ){
            int result = lhs.getAuthor().compareTo(rhs.getAuthor());
            // compare title if it's the same author
            if( result == 0 ){
    ...
    protected class OrderByDueDate implements Comparator<LibraryBookGeneric<Type>> {
        /**
         * Returns a negative value if lhs date is earlier than rhs. Returns a positive
         * value if lhs is later than rhs. Returns 0 if lhs and rhs are equal.
         */
        public int compare( LibraryBookGeneric<Type>lhs, LibraryBookGeneric<Type>rhs ){
